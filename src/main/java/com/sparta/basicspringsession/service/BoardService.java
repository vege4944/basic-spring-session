...
public class BoardService {
    ...
    public BoardDetailResponseDto getBoard(Long boardId) {
        Board board = boardRepository.findById(boardId).orElseThrow(()-> new NullPointerException("보드가 없어유"));
    ...
    @Transactional
    public BoardUpdateTitleRepsonseDto updateBoardTitle (
            Long boardId,
            BoardUpdateTitleRequestDto boardUpdateTitleRequestDto
    ) {
        Board board = boardRepository.findById(boardId).orElseThrow(()->new NullPointerException("보드없당"));

        board.updateTitle(boardUpdateTitleRequestDto.getTitle());

        return new BoardUpdateTitleRepsonseDto(
                board.getId(),
                board.getTitle(),
                board.getContents()
        );
    ...
    public BoardUpdateContentsResponseDto updateBoardContents (
        ...
        Board board = boardRepository.findById(boardId).orElseThrow(()->new NullPointerException("여기도없당"));
